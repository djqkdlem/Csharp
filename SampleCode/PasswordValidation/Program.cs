...
namespace PasswordValidation
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            System.Console.WriteLine(Union_Rule("123456Q$"));

            System.Console.WriteLine(Personal_Information("01093096116"));

            System.Console.WriteLine(oneString_Repetition("djqkdeeem"));

            System.Console.WriteLine(twoString_Repetition("123123"));

            System.Console.WriteLine(Keyboard_Continuity("12e45"));
        ...
        private static bool Union_Rule(string data)
        {
            bool NumberCH = false;
            bool CharCH = false;
        ...
        //회원정보로 DB에 저장된 사용자 정보를 변수에 저장하여 새로 변경할 패스워드에 포함 여부를 확인합니다. (Server Side Script를 이용하여 해당 변수가 소스코드에 노출되지 않게 적용)

        private static bool Personal_Information(string data)
        {
        ...
        private static bool oneString_Repetition(String data)
        {
        ...
        private static bool twoString_Repetition(String data)
        {
            for (int i = 0; i <= data.Length - 2; i++)
            {
                int count = 0;
        ...
        //키보드  연속성 체크(특수문자 제외)
        private static bool Keyboard_Continuity(String data)
        {
            String[] KeyboardCH = {
                            //숫자 연속성 체크
